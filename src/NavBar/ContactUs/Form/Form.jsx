...
const Form = () => {
  ...
  useEffect(() => {
    ...
    setRows(window.innerWidth > 1200 ? Math.round(0.00625 * window.innerWidth - 3.5): 4);
    window.onresize = () => {
      formDiv.style.width = `${window.innerWidth > 1200 ? 70 : -0.01429 * window.innerWidth + 87.14}%`;
      setRows(window.innerWidth > 1200 ? Math.round(0.00625 * window.innerWidth - 3.5): 4);
    };
  ...
  return (
    <div id="contactForm">
      <div>
        <h1 style={{marginTop: 0, marginBottom: '10px'}}>Contact Us</h1>
        <form ref={formRef} onSubmit={handleSubmit}>
          <div>
            <button type='reset' id="clearButton" onClick={handleReset}>
          ...
          <div>
            <input type='email' name='email' value={form.email} onChange={handleChange}  placeholder=" E-mail Address" required/>
            <input type='tel' name='phone' value={form.phone} onChange={handleChange}  placeholder=" Phone Number" required/>
          </div>
          <div>
            <textarea rows={rows} name='message' value={form.message} onChange={handleChange} placeholder=" Message" required/>
          </div>
        </form>
      </div>
    </div>
